...
export const updateSavedEvent = async (id: SavedEventId, savedEvent: UpdateSavedEventParams) => {
...
export const deleteSavedEvent = async (id: SavedEventId) => {
  const { id: savedEventId } = savedEventIdSchema.parse({ id });
  try {
    const [s] =  await db.delete(savedEvents).where(eq(savedEvents.id, savedEventId!))
    .returning();
    return { savedEvent: s };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
