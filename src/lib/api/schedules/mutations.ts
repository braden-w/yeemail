...
export const updateSchedule = async (id: ScheduleId, schedule: UpdateScheduleParams) => {
  ...
  try {
    const [s] =  await db
     ...
     .where(and(eq(schedules.id, scheduleId!), eq(schedules.userId, session?.user.id!)))
     .returning();
    return { schedule: s };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
...
export const deleteSchedule = async (id: ScheduleId) => {
  ...
  const { id: scheduleId } = scheduleIdSchema.parse({ id });
  try {
    const [s] =  await db.delete(schedules).where(and(eq(schedules.id, scheduleId!), eq(schedules.userId, session?.user.id!)))
    .returning();
    return { schedule: s };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
