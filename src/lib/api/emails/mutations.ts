...
export const createEmail = async (email: NewEmailParams) => {
  ...
  const newEmail = insertEmailSchema.parse({ ...email, userId: session?.user.id! });
  try {
...
export const updateEmail = async (id: EmailId, email: UpdateEmailParams) => {
  ...
  const newEmail = updateEmailSchema.parse({ ...email, userId: session?.user.id! });
  try {
    const [e] =  await db
     .update(emails)
...
export const deleteEmail = async (id: EmailId) => {
  ...
  const { id: emailId } = emailIdSchema.parse({ id });
  try {
    const [e] =  await db.delete(emails).where(and(eq(emails.id, emailId!), eq(emails.userId, session?.user.id!)))
    .returning();
    return { email: e };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
