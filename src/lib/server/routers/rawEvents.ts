...
import { createRawEvent, deleteRawEvent, updateRawEvent } from "@/lib/api/rawEvents/mutations";

export const rawEventsRouter = router({
  getRawEvents: publicProcedure.query(async () => {
    return getRawEvents();
  }),
  getRawEventById: publicProcedure.input(rawEventIdSchema).query(async ({ input }) => {
